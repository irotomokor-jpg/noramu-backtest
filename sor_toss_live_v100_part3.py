def reconcile(client: TossLiveClient, cfg: LiveConfig, state: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    exactly one known SINGLE MARKET stop or already covered by an active normal SELL
    order.  Missing/stale stop ids are recovered from broker OPEN conditionals.
    """
    ...
    for ticker in list(state.get("positions", {})):
        ...
        # adopt only a UNIQUE exact stop.  Never adopt an arbitrary app-created SINGLE.
        ...
        expire_near = True
        try:
            expire_near = pd.Timestamp(expire_s).date() <= today + timedelta(days=30)
        except Exception:
            expire_near = True
