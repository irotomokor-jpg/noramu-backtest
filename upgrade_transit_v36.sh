...
PIN="709b3bee701ddf61776a47c53d612a1219e649a0"
RAW="https://raw.githubusercontent.com/irotomokor-jpg/noramu-backtest/$PIN/v36_bundle"
EXPECTED_ZIP_SHA="ae6d3b46b2309c5df02d940bd306b19b9c31b516965a538541d3a93b1787210f"
TMP="$(mktemp -d /tmp/transit-v36.XXXXXX)"
BACK="$(mktemp -d /tmp/transit-v36-backup.XXXXXX)"
...
rollback(){
  echo "ERROR: Transit v3.6 validation failed; restoring previous files."
...
echo "[1/8] Downloading pinned Transit v3.6 bundle..."
: > "$TMP/transit-v36.b64"
for f in part00 part01 part02 part03 part04; do
  echo "  - $f"
  curl -fLsS --retry 3 --retry-delay 1 "$RAW/$f" >> "$TMP/transit-v36.b64"
done
...
if [[ "$ACTUAL_SHA" != "$EXPECTED_ZIP_SHA" ]]; then
  echo "ERROR: Transit v3.6 ZIP checksum mismatch"
...
# The archive is expected to contain backend/, frontend/, deploy/ at its root.
[[ -f "$TMP/pkg/backend/app.py" && -f "$TMP/pkg/frontend/app.js" ]] || {
  echo "ERROR: invalid Transit v3.6 bundle layout"
  exit 3
}
...
# Stop only after download/checksum/backup all succeeded.
...
echo "[4/8] Installing broader discovery, forward prediction, i18n search, and history collection..."
set +e
for rel in \
  backend/app.py \
...
if command -v node >/dev/null 2>&1; then
  node --check frontend/app.js >/dev/null || { rollback; exit 6; }
fi
.venv/bin/python -m backend.self_check || { rollback; exit 7; }

# Build/refresh holiday classification and coverage once before starting the 24/7 maintainer.
.venv/bin/python -m backend.history_maintainer --once --force-coverage || { rollback; exit 8; }
set -e

echo "[5/8] Installing 24/7 history-maintainer service..."
...
echo "[7/8] Verifying local web and asset version..."
curl -fsS --max-time 8 http://127.0.0.1:8103/ >/dev/null || { rollback; exit 10; }
grep -q 'v=190' frontend/index.html || { rollback; exit 11; }

echo "[8/8] Transit v3.6 ready"
echo "transit-web=$WEB"
echo "transit-collector=$COL"
echo "transit-history=$HIST"
echo "local-web-check=OK"
echo "fixes=remote-merge-search,full-serving-route-merge,forward-time-prediction,ja-en-station-search,rotating-history-breadth"
echo "assets=v190"
echo "NOTE: dense full-region history is still limited by the public API quota; v3.6 rotates breadth while keeping important/search routes denser."
echo "NOTE: full Japanese/English station coverage requires importing the official multilingual station-name XLSX with backend.import_station_names_xlsx."
